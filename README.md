...
사용자 이름을 기반으로 한 자동완성 검색 기능을 구현했습니다.

- `suggestions` 배열을 생성하여 중복 없는 사용자 이름 목록을 제공합니다.
- `Select` 컴포넌트를 사용하여 자동완성 UI를 구현했습니다.
- 검색 결과에 따라 동적으로 테이블 행을 필터링합니다.
...
커스텀 훅을 사용하여 API 호출을 추상화하고, 데이터 로딩 상태와 에러 처리를 관리합니다.

- useUser 훅을 통해 특정 사용자 데이터를 비동기적으로 가져옵니다.
- 로딩 상태와 에러 상태를 관리하여 UI에 적절히 반영할 수 있습니다.
...
react-i18next를 사용하여 다국어 지원을 구현했습니다. 이를 통해 사용자는 언어를 전환할 수 있으며, 애플리케이션 전체에서 일관된 언어 사용이 가능합니다.

- 한국어와 영어 리소스를 설정했습니다.
- 언어 감지 기능을 통해 사용자의 기본 언어를 자동으로 설정합니다.
...
공통 formatDate 함수를 사용하여 ISO 형식의 날짜 문자열을 일관된 형식으로 변환합니다. 이를 통해 애플리케이션 전체에서 일관된 날짜 표시가 가능합니다.
